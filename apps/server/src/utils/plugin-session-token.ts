...
export const SESSION_TOKEN_PREFIXES = ["Bearer", "bearer", "Bearer "];

export function getSessionTokenFromRequest(req: Request): string | null {
  const parseResult = z
    .object({ [SESSION_TOKEN_KEY]: z.string() })
  ...
  const { [SESSION_TOKEN_KEY]: authHeader } = parseResult.data;
  const [bearerPrefix, token] = authHeader.split(" ");

  if (!SESSION_TOKEN_PREFIXES.includes(bearerPrefix) || !token) {
    log.debug("invalid_session_token_format", { error: parseResult.error });
    return null;
  }
